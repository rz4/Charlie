...
(defn read-vocab []
  (if (os.path.exists VOCAB-PATH)
      ...
      (pd.DataFrame [[0 "[START]"] [1 "[END]"] [2 "[MASK]"] [3 "[UNKNOWN]"] [4 "[PAD]"]]
                    :columns (, 'TID 'TOKEN))))
...
(defmain [args]
  ...
  (let [ledger (read-ledger)
    ...
    (let [terms (-> ledger (get 'text) (. str) .split .explode .unique list)
          ...
          maxid (.max vocab.TID)]
      (print (.format ":Vocab-Size {}" maxid))
      (unless (< MAX-VOCAB maxid)
    ...
    ;- Load Language Model
    (setv model (load-model MAX-VOCAB 16 (if (os.path.exists MODEL-PATH) MODEL-PATH None))))

  ;- Train Language Model
  (let [epochs 1 batch-size 4
